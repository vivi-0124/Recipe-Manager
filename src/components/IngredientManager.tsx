  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.error(`材料の${operation === 'add' ? '追加' : '更新'}に失敗しました:`, errorData.error)
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                {/* 賞味期限 */}
                <div className="space-y-2">
                  <Label htmlFor="expiry_date" className="flex items-center gap-2 text-sm">
                ...
                {/* 単位 */}
                <div className="space-y-2">
                  <Label htmlFor="unit" className="text-sm">単位</Label>
                  <Input
                    id="unit"
                    type="text"
                    value={formData.unit}
      ...
      {ingredients.length === 0 ? (
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-8 sm:py-12">
            ...
            <h3 className="text-base sm:text-lg font-medium mb-2">材料がありません</h3>
            ...
            <Card key={category}>
              <CardHeader className="pb-3">
                <CardTitle className="flex items-center gap-2 text-lg">
                  ...
                  {category}
                  <Badge variant="secondary" className="text-xs">{categoryIngredients.length}個</Badge>
                </CardTitle>
              </CardHeader>
              <CardContent className="pt-0">
                <div className="space-y-2">
                  {categoryIngredients.map((ingredient) => {
                    ...
                    return (
                      <div key={ingredient.id} className="flex items-center justify-between py-3 px-3 rounded-lg border hover:bg-muted/50 transition-colors">
                        <div className="flex items-center gap-3 flex-1 min-w-0">
                          ...
                          <div className="flex-shrink-0">
                            ...
                            {expiryStatus === 'good' && ingredient.expiry_date && (
                            ...
                            <div className="flex items-center gap-3 text-xs text-muted-foreground">
                              {ingredient.expiry_date && (
                                <span className="flex items-center gap-1">
                                  ...
                                  {new Date(ingredient.expiry_date).toLocaleDateString('ja-JP')}
                                </span>
                              )}
                              {ingredient.notes && (
                                <span className="flex items-center gap-1 truncate">
                                  <FileText className="h-3 w-3 flex-shrink-0" />
                                  <span className="truncate">{ingredient.notes}</span>
                                </span>
                              )}
                            </div>
                          </div>
                          ...
                          <AlertDialog>
                            ...
                            <AlertDialogContent>
                              <AlertDialogHeader>
                                ...
                                <AlertDialogDescription>
                                  「{ingredient.name}」を削除します。この操作は取り消せません。
                                </AlertDialogDescription>
                              </AlertDialogHeader>
                              <AlertDialogFooter>
